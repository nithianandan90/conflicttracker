...
export default function ConflictUpdateForm(props) {
  const {
    id: idProp,
    conflict: conflictModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = conflictRecord
      ? { ...initialValues, ...conflictRecord }
      : initialValues;
    setName(cleanValues.Name);
    setDescription(cleanValues.Description);
    setTravel(cleanValues.Travel);
  ...
  const [conflictRecord, setConflictRecord] = React.useState(conflictModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Conflict, idProp)
        : conflictModelProp;
      setConflictRecord(record);
    };
    queryData();
  }, [idProp, conflictModelProp]);
  React.useEffect(resetStateValues, [conflictRecord]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Conflict.copyOf(conflictRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ConflictUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || conflictModelProp)}
