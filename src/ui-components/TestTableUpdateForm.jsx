...
export default function TestTableUpdateForm(props) {
  const {
    id: idProp,
    testTable: testTableModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = testTableRecord
      ? { ...initialValues, ...testTableRecord }
      : initialValues;
    setFlag(cleanValues.Flag);
    setName(cleanValues.Name);
    setCode(cleanValues.Code);
    setPopulation(cleanValues.Population);
    setConflictID(cleanValues.conflictID);
    setErrors({});
  };
  const [testTableRecord, setTestTableRecord] =
    React.useState(testTableModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(TestTable, idProp)
        : testTableModelProp;
      setTestTableRecord(record);
    };
    queryData();
  }, [idProp, testTableModelProp]);
  React.useEffect(resetStateValues, [testTableRecord]);
  const validations = {
    Flag: [],
    Name: [],
    Code: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            TestTable.copyOf(testTableRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "TestTableUpdateForm")}
      {...rest}
    >
      <TextField
        label="Flag"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || testTableModelProp)}
